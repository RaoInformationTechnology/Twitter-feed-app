        ...
        if (this.state.isAuthenticated == true) {
            ...
            navigator.geolocation.getCurrentPosition(location => {
                ...
                if (this.state.latitude && this.state.longitude) {
                    const trendsobj = {
                        latitudeobj: this.state.latitude,
                        longitudeobj: this.state.longitude
                    }
                    API.getTwitterTrends(trendsobj).
                    then((findresponse) => {
     ...
     render() {
         ...
         if (this.state.isAuthenticated == true) {
             ...
             if (!isFetching && !this.state.displaysearchtweets.length && this.state.tweet[0]) displaydate = this.state.tweet[0].slice(0, this.state.visible).map(tweet =>
                 ...
                 if (this.state.displaysearchtweets[0] && this.state.displaysearchtweets[0].data && this.state.displaysearchtweets[0].data.length) displaysearchtweetsview = this.state.displaysearchtweets[0].data.map(searchelement =>
                     ...
                     if (this.state.hashtag && this.state.hashtag.length) displayhashtag = this.state.hashtag.map(hashtagname =>
                         ...
                         if (!isLoaded) {
                             ...
                             <form action="/" method="POST" onSubmit={(e) => { this.handleClickSearch(e) }}>
                             <TextField
                 ...
                 {this.state.tweet[0] && !this.state.value ? <div>{this.state.visible < this.state.tweet[0].length &&
         ...
         <DialogContent>
         <DialogContentText>
         <form action="/" method="POST" onSubmit={(e) => { this.handleClickhashtag(e) }}>
         <TextField
         id="standard-full-width"
         label="Add Hashtag"
         style={{ margin: 8, width: 500 }}
